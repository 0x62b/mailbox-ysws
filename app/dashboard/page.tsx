...
/*
If you want to access this thing go onto https://localhost:3001/dashboard
*/
...
const items: Item[] = [
  ...
  },
  {
    ...
    description: `To improve <i>user experience</i>, I have made a landing page for
    rsvping and that stuff
    - HZ
      `,
    date: "Mar 17",
  },
...
export default function Home() {
  ...
  return (
    <div className="flex h-screen bg-zinc-900 text-gray-200">
      ...
      <section className="flex flex-col w-2/5 overflow-y-auto bg-zinc-800">
        {page === "inbox" ? (
          items.map((item) => (
            <button
              key={item.id}
              onClick={() => setSelected(item)}
              className={`flex p-4 gap-3 items-center hover:bg-zinc-700 transition text-start ${
                selected?.id === item.id ? "bg-zinc-700" : ""
              }`}
            >
          ...
          <div className="p-6 text-gray-400 text-sm">No messages here.</div>
        )}
      </section>

      {/* Detail Panel */}
      <main className="flex-1 p-6 overflow-y-auto max-w-full space-y-6 bg-zinc-700">
        {!selected || page !== "inbox" ? (
          <div className="text-gray-400 text-sm">Select a message to view details.</div>
        ) : (
          <>
            <section className="bg-zinc-800 border border-zinc-600 rounded-lg p-6 shadow-sm">
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  {selected.attachments.map((att, i) => (
                    <a
                      ...
                      href={att.url}
                      download
                      className="bg-zinc-600 px-3 py-1 rounded hover:bg-zinc-500 text-sm text-gray-300"
                    >
                      {att.name} ({att.size})
                    </a>
                  ))}
              ...
              <div className="mt-6 flex gap-3">
                <button className="px-3 py-1 rounded bg-zinc-600 hover:bg-zinc-500 transition text-sm">
                  Reply
                </button>
                <button className="px-3 py-1 rounded bg-zinc-600 hover:bg-zinc-500 transition text-sm">
                  Forward
            ...
            {selected.replies?.map((reply) => (
              <section
                key={reply.id}
                className="bg-zinc-800 border border-zinc-600 rounded-lg p-4 shadow-sm"
              >
